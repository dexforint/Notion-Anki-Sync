var NASConverter = (() => {
	...
	function rt(block, key) {
		const data = block[block.type] || {};
		return richTextToHtml(data[key] || data.rich_text || []);
	}
	...
	function renderBlock(block, ctx) {
		...
		switch (type) {
			...
			case "heading_1":
				return `<h1>${rt(block)}</h1>${renderChildren(children, ctx)}`;
			case "heading_2":
				return `<h2>${rt(block)}</h2>${renderChildren(children, ctx)}`;
			case "heading_3":
				return `<h3>${rt(block)}</h3>${renderChildren(children, ctx)}`;
	...
	async function convertToggle(block, options = {}) {
		if (block.type !== "toggle") {
			const err = new Error("This block is not a toggle");
			err.code = "not_toggle";
			throw err;
		}
		const ctx = {
		...
		let front = richTextToHtml(block.toggle?.rich_text || []) || "Untitled";
	...
	return { convertToggle, hashText, richTextToHtml };
})();
