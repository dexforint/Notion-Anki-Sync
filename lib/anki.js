var NASAnki = (() => {
	...
	async function listTags(url) {
		try {
			const tags = await invoke(url, "getTags");
	...
	async function setNoteTags(url, noteId, tags) {
		const clean = sanitizeTags(tags);
		try {
			await invoke(url, "updateNoteTags", { note: noteId, tags: clean });
			return;
		} catch (_) {}
		try {
			await invoke(url, "updateNote", { note: { id: noteId, tags: clean } });
			return;
		} catch (_) {}
		try {
			const info = await invoke(url, "notesInfo", { notes: [noteId] });
			const old = info?.[0]?.tags || [];
			const ref = String(noteId);
			if (old.length) await invoke(url, "removeTags", { notes: [ref], tags: old.join(" ") });
			if (clean.length) await invoke(url, "addTags", { notes: [ref], tags: clean.join(" ") });
		} catch (e) {
			console.warn("setNoteTags failed", e);
		}
	}
...
.nas-toggle.nas-toggle-h1 > summary .nas-h { font-size: 1.6em; font-weight: 700; }
.nas-toggle.nas-toggle-h2 > summary .nas-h { font-size: 1.35em; font-weight: 700; }
.nas-toggle.nas-toggle-h3 > summary .nas-h { font-size: 1.15em; font-weight: 700; }
.nas-toggle.nas-toggle-h4 > summary .nas-h { font-size: 1.05em; font-weight: 700; }
.nas-h { display: inline; }
...
`;
	...
	const deckCache = new Map(); // url -> { at, names:Set }
	const DECK_TTL = 5 * 60 * 1000;

	async function ensureDeck(url, deckName) {
		const cached = deckCache.get(url);
		if (cached && Date.now() - cached.at < DECK_TTL) {
			if (cached.names.has(deckName)) return;
			await invoke(url, "createDeck", { deck: deckName });
			cached.names.add(deckName);
			return;
		}
		const decks = await invoke(url, "deckNames");
		const entry = { at: Date.now(), names: new Set(decks) };
		deckCache.set(url, entry);
		if (!entry.names.has(deckName)) {
			await invoke(url, "createDeck", { deck: deckName });
			entry.names.add(deckName);
		}
	}
	...
	async function listTracked(url) {
		...
		for (const note of notes) {
			...
			out[id] = {
				...
				customTitle: String(fieldValue(note, "NotionTitle") || "").trim() || null,
	...
	const modelReady = new Map(); // url -> timestamp
	const MODEL_TTL = 5 * 60 * 1000;

	async function ensureModel(url, force = false) {
		const at = modelReady.get(url) || 0;
		if (!force && Date.now() - at < MODEL_TTL) return;
		const models = await invoke(url, "modelNames");
		if (!models.includes(MODEL_NAME)) {
			await invoke(url, "createModel", {
				modelName: MODEL_NAME,
				inOrderFields: ["Front", "Back", "NotionBlockId", "NotionTitle"],
				css: MODEL_CSS,
				cardTemplates: [{ Name: "Card", Front: "{{Front}}", Back: '{{FrontSide}}\n<hr id="answer">\n{{Back}}' }],
			});
			modelReady.set(url, Date.now());
			return;
		}
		try {
			const fields = await invoke(url, "modelFieldNames", { modelName: MODEL_NAME });
			if (Array.isArray(fields) && !fields.includes("NotionTitle")) {
				await invoke(url, "modelFieldAdd", { modelName: MODEL_NAME, fieldName: "NotionTitle" });
			}
		} catch (e) {
			console.warn("ensure NotionTitle field failed", e);
		}
		try {
			await invoke(url, "updateModelStyling", { model: { name: MODEL_NAME, css: MODEL_CSS } });
		} catch (e) {
	...
	async function addOrUpdate(url, { deckName, front, back, blockId, tags, moveDeck, customTitle, skipTags = false }) {
		await ensureDeck(url, deckName);
		await ensureModel(url);
		const cleanTags = tags ? sanitizeTags(tags) : ["notion"];
		const fields = { Front: front, Back: back, NotionBlockId: blockId, NotionTitle: customTitle || "" };
		const existing = await findNoteId(url, blockId);
		if (existing) {
			await invoke(url, "updateNoteFields", { note: { id: existing, fields } });
			if (moveDeck) {
				try {
			...
			if (tags && !skipTags) await setNoteTags(url, existing, cleanTags);
			return existing;
		}
	...
	async function listMedia(url, pattern = "nas_*") {
		try {
			const names = await invoke(url, "getMediaFilesNames", { pattern });
